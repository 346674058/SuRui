...
function search() {
	...
	$("#commodityListCon")
			.html(
					'<div class="loadingCon"><div class="loading left"></div>数据加载中...</div>');
	...
	$.ajax({
		...
		success : function(resp) {
			if (resp.indexOf("<htm") >= 0) {
				location.href=_BASE + "/error/404.htm";
			}

			$("#commodityListCon").html(resp);
		},
		error : function() {
			$("#commodityListCon").html(
					'<div class="loadingCon">系统忙，请稍后再试</div>');
		}
	});
}
