package com.myit.server.servlet;
...
/**
 * 名称：支付应答类 功能：servlet方式，建议前台通知和后台通知用两个类实现，后台通知进行商户的数据库等处理,前台通知实现客户浏览器跳转 类属性： 版本：1.0 日期：2011-03-11 作者：中国银联UPOP团队 版权：中国银联
...
public class PayCallbackServlet extends HttpServlet {
    ...
    private final static Logger LOGGER = Logger.getLogger(PayCallbackServlet.class);
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            response.getWriter().append("建议前台通知和后台通知用两个类实现，后台通知进行商户的数据库等处理,前台通知实现客户浏览器跳转<br>")
                    .append("签名是否正确：" + signatureCheck).append("<br>交易是否成功：" + "00".equals(resArr[10]));
