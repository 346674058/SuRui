...
import com.myit.server.util.Util;
...
public class IPSeeker {
                ...
                if (files[i].isFile()) {
                    if (files[i].getName().toLowerCase().equals(filename)) {
                        try {
                            ...
                            LOGGER.info("IP地址信息文件没有找到，IP显示功能将无法使用", e1);
                            ipFile = null;
                        }
                        break;
                    }
    ...
    public static void main(String[] args) {
        ...
        String ipStr=null;
        try {
            ipStr = Util.getIpAddress();
        } catch (UnknownHostException e) {
            System.out.println(e);
        }
        
        System.out.println(ip.getIPLocation(ipStr).getCountry() + ":"
                + ip.getIPLocation(ipStr).getArea());
    }
}
